...
client.on("qr", async qr => {
  ...
  console.log("Scan the QR from WhatsApp")
  console.log("================================")
...
client.on("ready", () => {
  ...
  try {
    if (client.info) {
      ...
      console.log(
        "WhatsApp name:",
        client.info.pushname || "unknown"
      )
    }
  } catch (err) {
    console.log(
      "Could not read WhatsApp info:",
      err.message
    )
  }
})
...
async function getBotSettings() {
  try {
    const res = await fetch(
      `${APP_URL}/api/branches/${BRANCH_ID}/bot-settings`,
      {
        headers: {
  ...
  try {
    ...
    const transcription =
      await openai.audio.transcriptions.create({
        ...
        language: "ar",
        prompt:
          "صالون تجميل، حجز، ميكب، تسريحة، عروسة، حمام مغربي، كيراتين، مانيكير، باديكير، سعر، موعد، فرع، خدمة، بشرة، بروتين، بليتش، هايلايت، سواريه، خطوبة، زفاف، حنة، فستان، إكسسوار، تسريحة عروسة، مكياج سواريه"
      })
        ...
        messages: [
          {
            ...
            content:
              `أنت متخصص في تحويل النصوص المكتوبة لنطق صوتي عربي طبيعي وسلس تمامًا.
مهامك بالترتيب:
...
4. أضِف تشكيلاً كاملاً ودقيقاً على كل كلمة عربية في النص
    ...
    if (
      ...
      try {
        ...
        if (!transcribed) {
          ...
          await msg.reply(
            "عذراً، مقدرتش أسمع الرسالة الصوتية. ممكن تكتبيلي؟ 😊"
          )
        ...
        if (settings.voiceReply) {
          ...
          console.log(
            "Voice conversion failed, falling back to text"
          )
        }
  ...
  async (req, res) => {
    ...
    if (
      ...
      return res
        ...
        .json({
          error:
            "phone and mediaBase64 required"
        })
    }
